...
def my_app(cfg : DepthConfig) -> None:
    ...
    checkpoint_reference = 'uy9ko/GLP_depth_decoder_finetune_contrast/model-Nyu_0.2_FDA_v2:v0'
    run = wandb.init(project=cfg.logger.proj_name)
    artifact = run.use_artifact(checkpoint_reference, type="model")
    artifact_dir = artifact.download()

    model=MyModel.load_from_checkpoint(os.path.join(artifact_dir, "model.ckpt"),
                                       lr= cfg.model.lr,
