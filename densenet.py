...
class _DenseLayer(nn.Module):
    def __init__(
        self, num_input_features: int, growth_rate: int, bn_size: int, drop_rate: float, memory_efficient: bool = False, iw: bool = False
    ) -> None:
        super().__init__()
        ...
        self.instance_norm_layer = InstanceWhitening(growth_rate)
        self.iw = iw
    ...
    def forward(self, input: Tensor) -> Tensor:  # noqa: F811
        ...
        # this place add iw to new_features and return x, w
        if self.iw:
            new_features, w = self.instance_norm_layer(new_features)
            return new_features, w
...
class _DenseBlock(nn.ModuleDict):
    ...
    def forward(self, init_features: Tensor) -> Tensor:
        ...
        for name, layer in self.items():
            if name == "denselayer" + str(self.num_layers):
                new_features, w = layer(features)
            else:
                new_features = layer(features)
            features.append(new_features)
        return torch.cat(features, 1), w
        ...
        self.features = nn.Sequential(
            OrderedDict(
                [
                    ...
                    ("norm0", InstanceWhitening(num_init_features)),
