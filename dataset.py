...
class SQUIDdataset(Dataset):
    def __init__(self, data_path: str, transform=None, is_predict=False, istrain=True): 
        ...
        self.total_filenames = os.listdir(self.image_path)
        random.seed(42)
        random.shuffle(self.total_filenames)
        if is_predict:
            self.filenames = self.total_filenames
        else:
            if istrain:
                self.filenames = self.total_filenames[:int(0.5*len(self.total_filenames))]
            else:
                self.filenames = self.total_filenames[int(0.5*len(self.total_filenames)):]
...
class lichongyi_dataset(Dataset):
    def __init__(self, data_path: str, transform=None, istrain=True):
        self.data_path = data_path
        self.transform = transform
        self.image_path = os.path.join(self.data_path, 'underwater_type_1')
        self.depth_path = os.path.join(self.data_path, 'transmission_type_1')
        self.filenames = os.listdir(self.image_path)
        random.shuffle(self.filenames)
        if istrain:
            self.filenames = self.filenames[:int(len(self.filenames)*0.8)]
        else:
            self.filenames = self.filenames[int(len(self.filenames)*0.8):]
    ...
    def __getitem__(self, idx: int) -> dict:
        filename = self.filenames[idx]
        image = Image.open(os.path.join(self.image_path, filename))
        depth = Image.open(os.path.join(self.depth_path, filename))
    ...
    def __init__(self, data_name, data_path, predict_data_path, scale_size, FDA_trans = False, batch_size=4, numworkers=0):
        ...
        if FDA_trans:
            ...
            self.train_transform = [
                ...
                A.FDA(reference_images=reference_images, beta_limit=0.1),
                A.HorizontalFlip(),
